...
async function collectM365Status() {
  try {
    ...
    const activeLicenses = licenseRes.data.value.filter(
      (l) => l.prepaidUnits.enabled > 0 && l.prepaidUnits.enabled < 1000000,
    );
    const mainLicense =
      activeLicenses.sort((a, b) => b.consumedUnits - a.consumedUnits)[0] || {};
    ...
    targetServices.forEach((srv) => {
      const item = healthRes.data.value.find((h) => h.service === srv.key) || {
        status: "serviceHealthy",
      };

      // กรองเฉพาะ Incident ที่ยังไม่แก้
      const activeIncidents = messagesRes.data.value.filter(
        (m) =>
          m.services.includes(srv.key) &&
          !m.isResolved &&
          m.classification === "incident",
      ).length;

      const isOperational = activeIncidents === 0;
      let resultText = isOperational
        ? "Healthy"
        : `${activeIncidents} Incident(s)`;
